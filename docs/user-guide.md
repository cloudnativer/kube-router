...
<<<<<<< HEAD
      ...
      --advertise-cluster-subnet string  If this parameter is set, the cluster IP of service will not be put into RIB, but the value set by this parameter will be put into RIB. The main purpose of this parameter is to reduce the number of service routes sent by kube-router to the upper BGP network devices.(This parameter needs to be set when "--advertise-cluster-ip=true" is set to take effect)
...
## Advertising Cluster Subnet
kube-router can advertise Cluster Subnet to BGP peers.
It does this by:
* advertising the Cluster Subnet to its BGP peers
To set the default for all services use the `--advertise-cluster-subnet` flags.

kube-router does not support annotations temporarily.The parameter of `--advertise-cluster-subnet` is mainly used to reduce the number of service routing entries in the online switch of k8s node.

Note: It is noteworthy that the `-advertise-cluster-subnet` parameter needs to be set when `-advertise-cluster-ip=true` is set to take effect.
